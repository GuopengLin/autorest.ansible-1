...
doc to get familiar with concepts like Extension, plugin, jsonrpc, codeModel, etc. The main purpose of this code generator is to faciliate the development of new ansible modules,
...
The strong similarity among these anisble modules justify the feasibility of writing an ansible codegen. 
And we can use one sentence to roughly describe how this codegen works:

  **Transform and enrich the codeModel emittted by autorest python autorest extension into a container class called AnsibleCodeModel, then using the
...
Arguspec spec is the ansible terms for module parameter schema. And ansible documentation is just schema with more description and unique format requirements. The compsoing logic for module documentation and module arguspec are quite similar. They all use the Module options(parameters) defined in Ansible codeModel as input, recursively transform them into the schemas required by ansible modules.
...
azure python sdks are generated by autorest.python extension. They are cerntain rules to name the crud functions which representing the underlie http verbs. You may looked at the following two function signaures to get a sense of the naming convention used by the autorest.python.
...
for most the cases, you'll find a create_or_update function used in both create and update  resource; or a seperate create and a seperate update function used in create and update resource accordingly. And a delete function for delete resource if delete is supported for the resource. Following this convention, we can easily find the Method in AnsibleCodeModel and required the method call parameters as well.

The following two snippets pretty much self-explains how to composing the "create and update" methods call. See [here] (../src/plugins/Ansible/AnsibleModuleSdk.ts) for more details.
